...
impl Process {
    ...
    pub(super) fn start_call_block(&mut self, block: &Call) -> Result<(), ExecutionError> {
        ...
        // start new execution context in for the operand stack. this has the effect of
        // resetting stack depth to 16.
    ...
    pub(super) fn end_call_block(&mut self, block: &Call) -> Result<(), ExecutionError> {
        ...
        self.system.set_fmp(ctx_info.fmp);
        self.stack
            .restore_context(ctx_info.stack_depth as usize, ctx_info.next_overflow_addr);
