...
use vm_core::utils::{group_vector_elements, RandomCoin, IntoBytes};
use vm_core::{chiplets::hasher::Hasher as MidenHasher, QuadExtension};
use winter_fri::{
    folding::fold_positions, FriOptions,
...
> {
    ...
    match result {
        Ok(res) => return Ok((res.0, res.1, res.2, commitments)),
...
fn iterate_query_fold_2_quad_ext(
    ...
    for depth in 0..number_of_layers {
        ...
        //let query_values = [
            //QuadExt::new(query_values[3], query_values[2]),
            //QuadExt::new(query_values[1], query_values[0]),
        //];
